...
- Commit each verified work unit and record the hash in the progress file. After a rebase or squash, a stale hash is informational, not an error.
...
- The file must answer: current phase/task, next allowed action, which items are done/pending/blocked, which commit and verification prove each done item, and what residual risk remains.
