...
- Each item needs: title, binary acceptance, verification command or check, status, and commit slot.
...
- [ ] 1. <work unit> — accept: <binary check> — verify: `<command>` — commit:
- [ ] 2. <work unit> — accept: <binary check> — verify: `<command>` — commit:
```
...
- Every turn: read the ledger, process only the next batch, update the ledger, verify, and commit if files changed.
...
The executing agent may only flip `status`, fill `evidence`, `notes`, `verification`, `commit`, and advance `current_batch`. Item ids and rules are read-only during execution.
