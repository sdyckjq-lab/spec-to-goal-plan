  ...
  "phases": [
    {
      ...
      "acceptance": [],
      "tasks": [
        { "id": "1.1", "title": "", "status": "pending", "verification": [], "commit": null }
      ]
    }
  ],
  "turn_log": []
...
The executing agent may only flip `status`, fill `verification`, `commit`, `decision_log`, and `turn_log` entries. Task definitions, acceptance, and rules are read-only during execution.
